...
onehotY = zeros(size(y,1),num_labels);
for i=1:m,
  for j = 1:num_labels,
    if(y(i) == j)
      onehotY(i,j) = 1;
...
J = 1/m * (sum(sum(-1.*onehotY.*log((a3))-(1.-onehotY).*log(1.-(a3))))) + (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
D1 = zeros(size(a2,2)-1,size(X,2));
D2 = zeros(size(a3,2),size(a2,2));
for i=1:m,
  delta3 = (a3(i,:))' - (onehotY(i,:))';
